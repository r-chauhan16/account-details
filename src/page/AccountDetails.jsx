...
function AccountDetails() {
    ...
    const { roles, locale } = useUserInfo();
    ...
    // for pending aids
    const erpId = roles[roles.length - 2];
    const [pdata, setpData] = useState([]);
    const [pdataFetched, setpDataFetched] = useState(false);

    // console.log(pdata);
    useEffect(() => {
        const fetchpData = async () => {
            try {
                const customApi = `https://oradevl.lasierra.edu:9443/api/pendingaid?erpid=${erpId}`;
                // const customApi = `https://oradevl.lasierra.edu:9443/api/pendingaid?erpid=${erpId}`;
                console.log('API_URL:', customApi);
    ...
    }, [erpId, pdataFetched, setpDataFetched]);
                    ...
                    {summary && (
                        <div className={classes.amountBoxRow}>
                            <div className={classes.amountBox}>
                                ...
                                <div className={classes.amountRow}>
                                    <Typography variant={'h4'} component={'div'}>
                                        {intl.formatMessage({ id: 'PendingAids' })}
                                    </Typography>
                                    <Typography variant={'body2'} component={'div'} className={classes.amount}>
                                        {pdata.map((innerArray, index) => (
                                            innerArray && (
                                                <div key={index}>
                                                    -{currentyFormater.format(innerArray[7] + innerArray[8])}
                                                </div>
                                            )
                                        ))}
                                    </Typography>
                                ...
                                <div className={classes.amountRow}>
                                    <Typography variant={'h4'} component={'div'}>
                                        {intl.formatMessage({ id: 'yourResponsiblity' })}
                                    </Typography>
                                    <Typography variant={'body2'} component={'div'} className={classes.amount}>
                                        {pdata.map((innerArray, index) => (
                                            innerArray && (
                                                <div key={index}>
                                                    {currentyFormater.format(summary.accountBalance - innerArray[7] - innerArray[8])}
                                                </div>
                                            )
                                        ))}
                                    </Typography>
                    ...
                    {Array.isArray(transactions) && transactions.length > 0 && (
                        <div className={classes.transactionsBox}>
                            <Typography variant={'h4'} component={'div'} className={classes.recentTransactions}>
                                {intl.formatMessage({ id: 'AccountDetails.transactionHistory' })} {lastDate} - {firstDate}
                            </Typography>
...
function AccountDetailsWithProviders() {
    ...
    const aidserviceURL = 'https://rklj2lb5ak.execute-api.us-east-1.amazonaws.com/api';
    console.log(aidserviceURL);
    ...
    //    queryFunction: experienceTokenQuery,
    //    queryParameters: { serviceUrl : 'https://rklj2lb5ak.execute-api.us-east-1.amazonaws.com/api' },
    //    resource: 'award-maintenance'
    // }));
